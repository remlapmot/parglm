# Authors and Citation

## Authors

- **Benjamin Christoffersen**. Author.
  [](https://orcid.org/0000-0002-7182-1346)

- **Anthony Williams**. Copyright holder.

- **Boost developers**. Copyright holder.

- **Tom Palmer**. Author, maintainer.
  [](https://orcid.org/0000-0003-4655-4511)

## Citation

Source:
[`DESCRIPTION`](https://github.com/remlapmot/parglm/blob/0.2.0/DESCRIPTION)

Christoffersen B, Palmer T (2026). *parglm: Parallel GLM*. R package
version 0.2.0, <https://github.com/remlapmot/parglm>.

    @Manual{,
      title = {parglm: Parallel GLM},
      author = {Benjamin Christoffersen and Tom Palmer},
      year = {2026},
      note = {R package version 0.2.0},
      url = {https://github.com/remlapmot/parglm},
    }
